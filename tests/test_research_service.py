...
def test_research_service_rejects_failed_internal_episode_after_archiving(
    ...
    import tools.research_service as research_service
    ...
    def fake_popen(command, **kwargs):
        return real_popen(
            [sys.executable, str(echo_script), *command[1:]],
            **kwargs,
        )

    monkeypatch.setattr(research_service.shutil, "which", lambda name: sys.executable)
    monkeypatch.setattr(research_service.subprocess, "Popen", fake_popen)
    monkeypatch.setattr(
        service,
        "_parse_summary",
        lambda output: {
