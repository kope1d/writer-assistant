...
def test_project_search_maps_lightrag_chunks_back_to_scoped_source_lines(tmp_path: Path):
    ...
    assert captured["query"].startswith("Writer Assistant 资料范围：角色\n")
    assert "Writer Assistant 资料范围" not in payload["results"][0]["snippet"]
...
def test_lightrag_manifest_tracks_incremental_insert_update_and_delete(tmp_path: Path):
    ...
    assert "Writer Assistant 资料范围：作品核心" in rag.inserted[0][2]
    assert "Writer Assistant 子分类：故事基础" in rag.inserted[0][2]
