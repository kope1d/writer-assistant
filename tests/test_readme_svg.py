...
def test_readme_documents_studio_as_primary_entry():
    ...
    assert "writer studio" in text
    assert "Studio 是 Writer Assistant 首推的日常入口" in text
...
def test_skill_docs_no_longer_present_agent_as_primary_entry():
    ...
    assert "`writer dante` 是主编排入口" in root_skill
    assert "`write` / `multi-write` / `review`" in root_skill
    assert "`writer agent` 是主编排入口" not in root_skill
    assert "writer dante" in goethe_skill
    assert "writer agent \"写第一章\"" not in goethe_skill
