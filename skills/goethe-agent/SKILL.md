...
对大多数用户来说，日常只需要记住两个入口：`writer goethe` 做 planning，`writer dante` 写正文。
...
- 持续修订背景/设定/人物/大纲
- 生成并审阅 source pack
- 在资产满足条件后显式 handoff 给 `writer dante`
...
6. 持续整理人物、设定和大纲
7. 资产达到写作条件后，提示用户切到 `writer dante`
...
- 询问用户是否有自己提供的文本
- 告诉用户后续可以用 `writer style extract <source_id> --source <file>`
