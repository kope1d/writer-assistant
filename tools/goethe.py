...
GOETHE_TOOL_DESCRIPTIONS: dict[str, str] = {
    ...
    "generate_character_draft": (
        "生成符合 Writer Assistant 角色文档结构的完整草案；只写 planning。"
    ),
...
DEFAULT_GOETHE_SYSTEM_PROMPT = f"""你是 Writer Assistant 的 Goethe，长期会话规划 Agent。

你的职责是汇总灵感、提出建议、收敛人物/设定/大纲，并把确认后的资产整理成可写内容。
正文推进交给 Dante。项目目录通常已由 Studio 或 `writer init` 创建，你不负责建目录。
...
class GoetheChatAgent:
    ...
    def run(self) -> GoetheResult:
        ...
        print("\n" + "=" * 50)
        print("   Writer Assistant Goethe 长会话规划 Agent")
...
def run_goethe() -> int:
    ...
    if not config_path.exists():
        print(
            "❌ 当前目录不是小说项目。\n"
            "请先运行 `writer init <novel_id>` 或 `writer studio` 创建作品，"
            "再进入 Goethe 规划会话。"
        )
        return 1
