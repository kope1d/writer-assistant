...
class StudioApplication:
    ...
    def open_project(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if not target.is_dir() or not (target / "novel_config.yaml").is_file():
            raise StudioError("所选目录不是有效的 Writer Assistant 作品项目")
...
class LegacyStudioRequestHandler(SimpleHTTPRequestHandler):
    server_version = "WriterAssistant/0.1"
...
def create_server(
    ...
    *,
    ...
    if missing_assets:
        raise StudioError(
            "Writer Assistant 安装不完整，缺少 Studio 核心资源: " + ", ".join(missing_assets),
            HTTPStatus.INTERNAL_SERVER_ERROR,
...
def run_studio(
    ...
    *,
    ...
    print(f"Writer Assistant Studio: {url}")
