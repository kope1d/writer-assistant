"""Writer Assistant CLI - 命令行接口

用法:
    writer init <novel_id>     # 初始化项目
    writer sync                # 同步 src -> data
    writer write <chapter>     # 写章节
    writer review <chapter>    # 审查章节
    writer context <chapter>   # 构建上下文
    writer style extract       # 提取风格
    writer status             # 查看状态
    writer --help            # 显示帮助
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="writer",
        description="Writer Assistant 长篇小说创作引擎",
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"Writer Assistant {__version__}",
    )
...
def _add_asset_command(subparsers):
    """asset 命令 - 结构化资产与 Writer Assistant Asset Package。"""
...
def _add_agent_command(subparsers):
    ...
    subparsers.add_parser(
        "agent",
        help="已退役：请改用 writer dante",
        description="已退役：请改用 writer dante",
    )
...
def _cmd_context(args) -> int:
    ...
    if inspection is None:
        ...
        for name in sections:
            logger.info(f"  - {name}")
        logger.info("检查实际 Agent 输入: writer context <chapter> --agent writer --show")
        return 0
...
def _cmd_style_synthesize(args) -> int:
    ...
    if not config and getattr(args, "novel_id", "current") == "current":
        logger.error("未找到 novel_config.yaml，请先运行 writer init")
        return 1
...
def _cmd_agent(args) -> int:
    """agent 命令 - 已退役"""
    logger.error("writer agent 已退役，请改用 writer dante。")
    return 1
...
def _build_sync_suggestions(status: dict) -> list[str]:
    ...
    if status["outline_pending"]:
        messages.append("大纲源文件有更新，运行 `writer sync` 以刷新 data/hierarchy.yaml")

    if status["missing_cards"]:
        preview = ", ".join(status["missing_cards"][:5])
        messages.append(
            f"存在缺失角色卡片（{preview}），运行 `writer sync` "
    ...
    if status.get("stale_cards"):
        ...
        messages.append(
            f"存在过期角色卡片（{preview}），运行 `writer sync` "
