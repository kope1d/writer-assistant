...
function showBootstrapError(error) {
  ...
  if (location) location.textContent = "Writer Assistant Studio 仍可切换作品";
...
function suggestProjectPath() {
  ...
  const update = () => {
    ...
    const slug = title
      ...
      .replace(/^_|_$/g, "")
      .slice(0, 40) || "my_novel";
    const defaultPath = `../WriterAssistantNovels/${slug}`;
...
const productTourSteps = {
  workspace: {
    ...
    title: "先认识你的写作工作台",
    copy: "这里不是只和 AI 聊天的页面。Writer Assistant 把策划、资料、正文、校验和写作都放进同一个作品工作区；先花半分钟看一圈，再开始创建你的故事。",
    next: "浏览工作区",
    nextStep: "navigation",
  },
  navigation: {
  ...
  write: {
    ...
    const productTourStorageKey = "writer-product-tour-v2";
...
function renderRuntimeSkillCatalog() {
  ...
  state.runtimeSkill.skills.forEach((skill) => {
    ...
    [
      ...
      skill.source_format === "standard-skill-md" ? "标准 SKILL.md" : "Writer Assistant manifest",
...
function renderAgentHistory(agent, history, sessionId = "default") {
  ...
  if (!messages.length) {
    appendChatMessage("assistant", "Writer Assistant", agentEmptyGuidance(agent));
    return;
  }
...
function toggleInspectorCollapsed(collapsed, options = {}) {
  ...
  if (persist) {
    state.outlineInspectorAutoCollapsed = false;
    writeLocalValue("writer-inspector-collapsed", collapsed ? "1" : "0");
  }
}
...
function applyTheme(theme) {
  ...
  setMarkdownEditorTheme(theme);
    writeLocalValue("writer-theme", theme);
}
...
async function start() {
    const storedTheme = readLocalValue("writer-theme");
  ...
  try {
    ...
    const inspectorPreference = readLocalValue("writer-inspector-collapsed");
