...
class LightRAGConfiguration:
    ...
    def from_runtime(cls) -> LightRAGConfiguration:
        ...
        if embedding_provider == "local" and importlib.util.find_spec("fastembed") is None:
            raise SearchConfigurationError(
                "本地 Embedding 依赖未安装，请重新运行 Writer Assistant 启动器"
            )
...
class LightRAGSearchBackend:
    ...
    def _create_rag(self) -> Any:
        ...
        try:
            ...
            raise SearchConfigurationError(
                "LightRAG 依赖未安装，请重新运行 Writer Assistant 启动器安装依赖"
            ) from exc
        ...
        if config.provider == "anthropic" and config.query_mode != "naive":
            try:
                ...
                raise SearchConfigurationError(
                    "LightRAG 的 Anthropic 适配器未安装，请重新运行 Writer Assistant 启动器"
                ) from exc
    ...
    def _rag_document_body(document: IndexedDocument) -> str:
        ...
        return (
            f"Writer Assistant 资料范围：{SCOPE_LABELS.get(document.scope, document.scope)}\n"
            f"Writer Assistant 子分类：{document.category_label}\n"
            f"Writer Assistant 来源：{document.path}\n\n"
            f"{document.body}"
        )
...
class ProjectSearchIndex:
    ...
    def _semantic_query(query: str, scope: str) -> str:
        if scope == "all":
            return query
        return f"Writer Assistant 资料范围：{SCOPE_LABELS[scope]}\n{query}"
    ...
    def _strip_rag_metadata(chunk: str) -> str:
        prefixes = (
            "Writer Assistant 资料范围：",
            "Writer Assistant 子分类：",
            "Writer Assistant 来源：",
        )
