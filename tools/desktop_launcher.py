...
def check_for_source_update(
    ...
    *,
    ...
    try:
        ...
        print("[Writer Assistant] 正在检查源码更新……")
...
def ensure_runtime(root: Path) -> Path:
    ...
    if root.resolve() == installed_root and not (root / "pyproject.toml").is_file():
        ...
        if _installation_healthy(installed_python, root, check_dependencies=False):
            print("[Writer Assistant] 已安装环境与依赖已就绪。")
            return installed_python
        raise LauncherError("当前 Writer Assistant 安装不完整，请重新安装后再启动。")
    ...
    print("[Writer Assistant] 正在检查并下载所需依赖，首次运行可能需要几分钟……")
    try:
        _run(
            [
                python,
    ...
    print("[Writer Assistant] 依赖安装与自检完成。")
    return python
...
def _is_openwrite_server(port: int) -> bool:
    ...
    try:
        ...
        return False
    return server.startswith("WriterAssistant/") and payload == {"ok": True}
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if sys.version_info < MIN_PYTHON:
        print("[Writer Assistant] 需要 Python 3.10 或更高版本。")
        return 2
    ...
    try:
        if not args.no_update:
            ...
            if update.message:
                print(f"[Writer Assistant] {update.message}")
        ...
        if already_running:
            print(f"[Writer Assistant] Studio 已在运行：{url}")
            webbrowser.open(url)
            return 0
        ...
        print(f"[Writer Assistant] 正在启动 Studio：{url}")
        return _run(command, cwd=root, check=False).returncode
    except LauncherError as exc:
        print(f"\n[Writer Assistant] 启动失败：{exc}")
        return 1
